    ...
    if data.get("status") == "success" and pokemons:
        for pokemon in pokemons:
            ...
            if pokeID in wanted or pokeID > 151:
                ...
                dist = str(int(haversine(LON, LAT, lonn, latt)*1000))
                id = pokemon.get("id")
                if id not in found:
                    found.append(id)
                    out = "["+strftime("%H:%M")+"] "+"Found a " + name + " " + dist + "m away! Url: " + view_url + str(latt)+","+str(lonn)+" with id: "+str(id)
